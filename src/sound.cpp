...
bool Sound::initOpenAL()
{
   ...
   if (device != NULL) 
   {
      ...
      {
         Log::add("Sound::initOpenAL() Couldn't create context!");
      }

   }
   else
   {
      Log::add("Sound::initOpenAL() No OpenAL device available!");
   }
   enabled = false;
   timer.reset();
   return(false);
...
void Sound::flush()
{
   ...
   if(backMusic)
   {
      if(!backMusic->update()) 
      {
         Log::add("Sound::flush: Error while playing music");
         delete backMusic;
         backMusic = NULL;
      }
   }
